...
class MessagesFinal extends Component {
  ...
  recMsg() {
    ...
    let persons = [];
    let up = [];
    axios.get(`http://54.237.17.61:8080/communication/message/find`, { params: { senderId: this.state.myId, receiverId: this.state.addedUserId } })
      .then(res => {
        persons = res.data;
        persons.map((item, i) => {
  ...
  sendMsg = () => {
    if (this.state.msgTypo != "") {
      ...
      axios.post('http://54.237.17.61:8080/communication/message/send', {
        "senderId": sender,
