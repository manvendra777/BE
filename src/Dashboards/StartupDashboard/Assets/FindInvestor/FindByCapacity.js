...
class FindByCapacity extends Component {
    ...
    getListData() {
        ...
        //54.237.17.61/investor/profile/find?min=100&max=199
        this.setState({ InvestorList: [] })
        var investor;
        axios.get(`http://54.237.17.61/investor/profile/find`, { params: { min: this.state.value[0],max:this.state.value[1] } })
            .then(res => {
                investor = res.data;
                investor.map((item, i) => {
                    console.log(item);
